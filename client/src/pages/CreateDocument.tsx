    ...
    return (
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
          <Card size="lg" variant="selectable" className="cursor-pointer" onClick={() => setMode('upload')}>
            <CardContent className="flex flex-col items-center justify-center py-8">
              ...
              <h3 className="text-base font-bold text-slate-900">Upload PDF</h3>
              <p className="mt-2 text-xs text-slate-600 text-center">Upload an existing PDF document</p>
            </CardContent>
          </Card>

          <Card size="lg" variant="selectable" className="cursor-pointer" onClick={() => setMode('edit-pdf')}>
            <CardContent className="flex flex-col items-center justify-center py-8">
              ...
              <h3 className="text-base font-bold text-slate-900">Annotate PDF</h3>
              <p className="mt-2 text-xs text-slate-600 text-center">Add text and images to a PDF</p>
            </CardContent>
          </Card>

          <Card size="lg" variant="selectable" className="cursor-pointer" onClick={loadTemplate}>
            <CardContent className="flex flex-col items-center justify-center py-8">
              ...
              <h3 className="text-base font-bold text-slate-900">Use Template</h3>
              <p className="mt-2 text-xs text-slate-600 text-center">Start with company template</p>
            </CardContent>
          </Card>
        </div>
      </div>
    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-6">
          ...
          <Button variant="secondary" onClick={() => setMode('select')}>Back</Button>
        </div>
        ...
        <Card className="rounded-lg">
          <CardContent className="space-y-4">
            ...
            >
              ...
              {isDragActive ? (
                <p className="mt-2 text-blue-600 font-medium">Drop the PDF here...</p>
              ) : (
                <>
                  <p className="mt-2 text-slate-600 font-medium">Drag and drop a PDF here, or click to select</p>
        ...
        <Card className="rounded-lg">
          <CardContent className="space-y-4">
            <div
              {...getEditPdfRootProps()}
              className={`border-2 border-dashed rounded-lg p-12 text-center cursor-pointer transition-all ${
                isEditPdfDragActive ? 'border-purple-500 bg-purple-50' : 'border-slate-300 hover:border-purple-400'
              }`}
            >
              ...
              {isEditPdfDragActive ? (
                <p className="mt-2 text-purple-600 font-medium">Drop the PDF here...</p>
              ) : (
                <>
                  <p className="mt-2 text-slate-600 font-medium">Drag and drop a PDF here, or click to select</p>
                  <p className="mt-1 text-xs text-slate-500">Maximum file size: 10MB</p>
                  <p className="mt-3 text-sm font-medium text-purple-600">Add text annotations and images</p>
                </>
              )}
            </div>
          </CardContent>
    ...
    return (
      <div className="fixed inset-0 z-50 bg-white">
        <div className="h-full flex flex-col">
          <div className="bg-purple-600 px-6 py-4 flex items-center justify-between shadow-sm">
            <div className="flex-1">
              <h1 className="text-lg font-bold text-white mb-2">Annotate PDF</h1>
              <Input
                value={title}
                onChange={(e) => setTitle(e.target.value)}
